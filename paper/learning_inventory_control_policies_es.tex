...
leaderboard claim: the one-warehouse multi-retailer policy robustly beats the same-protocol tuned
gate on the asymmetric three-retailer ($+4.61\%$), high-CV ten-retailer ($+7.07\%$), and the
previously-unsolved strongly heterogeneous ten-retailer ($+12.44\%$, five-seed mean, all seeds below
the gate) instances but does not robustly beat the published PPO scalar (it sits $\approx3\%$ above
PPO on the hardest row); the ameliorating-inventory result beats a purchase-only gate while remaining
a large gap-to-bound; and the production/assembly/distribution network is reported as
learned-versus-own-heuristic research, robustly beating the environment's own pairwise base-stock gate
on the serial, pure-assembly, and --- via a new residual gate-backbone action head --- mixed topologies
($-2.20\%$, five-seed mean, all seeds below the gate). The hardest-instance multi-retailer geometry (a
constant leaf) was found by a search over the decoder classes, scored by the same seed-robust gate.
...
\begin{enumerate}
    ...
    multi-retailer it gives seed-robust same-protocol gate beats on the asymmetric three-retailer,
    ...
    topologies, the last via a new residual gate-backbone action head that upgrades the former
    gate-match to a seed-robust $-2.20\%$ beat. The two hardest-instance geometries were found by a
    search over the decoder classes, scored by the same seed-robust gate.
...
the multi-echelon and one-warehouse problems, where the structured gate is well short of optimal and
...
(setting~2), with all $5$ seeds below the gate on both settings, so the beat is robust to optimizer
...
env's own grid-searched best constant base-stock gate (the same-protocol comparator, seed-mean
$910.3\pm0.5$ / $1138.0\pm0.4$); all $5$ seeds beat the gate on both settings. The published A3C row is
...
\emph{beats} the gate, and the beat is seed-robust: over $5$ independent CMA-ES seeds the FIFO
improvement is $+1.171\%\pm0.002\%$ and the LIFO improvement is $+0.840\%\pm0.034\%$, with all $5$
seeds beating the gate on both (the cross-seed standard deviation is negligible against the mean
...
Improvement over gate $\Delta\%$ ($5$-seed mean) & $\mathbf{+1.171\%\pm0.002\%}$ & $\mathbf{+0.840\%\pm0.034\%}$ \\
...
standard deviation over $5$ independent CMA-ES seeds (all $5$ beat the gate on both instances);
...
benchmark ($10{,}354.8$), with all $5$ independent CMA-ES seeds below the gate. The cross-seed standard
...
$7{,}963.6$; all $5$ below the gate, each $\gg 2\times$ its own evaluation SEM). The like-for-like
...
independent optimizer seeds (all $5$ below the gate).
...
$2{,}489.0$; all $5$ below the gate). The like-for-like comparator is the constant node-base-stock
...
gate beat ($5$-seed mean, all five seeds below the gate). A raw per-retailer order geometry without a gate-reproducing anchor loses by $-50.84\%$,
...
geometry --- is what robustly beats the gate there, turning a long-standing tie into a $+12.44\%$
gate beat (Section~\ref{sec:owmr-results}).
...
Section~\ref{sec:methods-eval}: the gate grid is searched on a $64$-path block (argmin verified
stable against $96/256$), and learned policy and gate are re-scored on a disjoint $4096$-path held-out
...
incumbent and the gate-reproducing warm-start anchor.
...
Table~\ref{tab:owmr-results} reports the learned soft tree against the tuned gate on the held-out
block. The seed-robust headline is the asymmetric $K{=}3$ row: a depth-2 axis-aligned linear-leaf
tree, freshly warm-started at the gate and trained with gentle $\sigma=0.05$ on $24$ common-random
training paths per generation, improves the $100$-period cost from $1169.59$ to
$1115.67\pm6.13$ across five independent CMA seeds ($+4.61\%$ against the gate; all five seeds beat
it). The training protocol is the load-bearing lever: fresh gate warm-start, moderate initial
...
same-protocol gate $91890.25$ to a $5$-seed mean $85395\pm1031$ ($+7.07\%$ against the gate; all six
seeds beat it), so this row is now a seed-robust gate beat. The hardest, strongly heterogeneous
$K{=}10$ instance --- which an earlier hand-search could only \emph{tie} (the trained incumbent landed
above the strong asymmetric gate within budget, so the deployed policy fell back to the gate-reproducing
anchor) --- is now also a robust gate beat. A search over the decoder classes
...
$100$-period cost from the same-protocol gate $50445.20$ to a five-seed mean $44170.26\pm450.4$
($+12.44\%$ against the gate; all five seeds below it, $\mathrm{std}\approx1.0\%$ of the mean). The
former tie was therefore \emph{action-design-limited}, not representation- or search-limited: the
gate lies inside the learned policy's class, but only the constant-leaf geometry --- which the
...
the learned soft tree \emph{robustly beats the strongest same-protocol tuned heuristic gate} on all
...
base-stock $+$ allocation gate, common-random-number evaluation ($4096$ held-out paths, horizon
$100$). Costs are $100$-period undiscounted totals (lower is better). $\Delta\%$ is the learned
policy's improvement over the gate. The first two rows report optimizer-seed mean $\pm$ standard
deviation over $6$ independent CMA seeds (all $6$ beat the gate); the third row is the deployed
warm-start floor. The published PPO column is the strongest \emph{learned} reference of
\citet{kaynov2024owmr}, reported as cross-protocol context rather than a keep/discard gate.}
\label{tab:owmr-results}
\begin{tabular}{llcccc}
\toprule
Instance & Evidence & Learned & Gate & $\Delta\%$ & Published PPO \\
\midrule
...
the published base-stock). All three learned rows are seed-robust gate beats: every seed beats the gate
...
echelon-targets) vs.\ the strongest tuned echelon base-stock $+$ allocation gate,
...
order-up-to gate): the tighter capacity curbs holding on idle stock, and the gap to the (now lower)
bound closes slightly to $92.2\%$. As in the anchor rows, the win is the like-for-like beat of the
tuned order-up-to gate; the LP value remains an upper bound, reported with its gap and never treated
as achievable.
...
\paragraph{A residual gate-backbone action head.}
On the richer \emph{mixed} distribution-and-assembly topology the flow warm-start is not gate-invertible:
the scale-normalized leaf cannot reproduce the tuned pairwise base-stock gate exactly, so CMA-ES seeds
start off-gate and scatter widely, leaving the row only at parity (Section~\ref{sec:pirhoo-results}). The
fix follows a simple design principle: rather than relying on the optimizer to \emph{rediscover} the
gate from an arbitrary initialization, we build the gate \emph{into} the policy as its backbone and let
the search learn only a small correction on top of it. Concretely, we introduce a \emph{residual
gate-backbone} action head, a new action geometry in which the learned policy outputs a signed
correction \emph{around} the gate rather than the order itself:
\begin{equation}
    \label{eq:residual-head}
    q^{r}_t \;=\; \mathrm{clamp}\!\bigl(\,q^{\mathrm{gate},r}_t + \mathrm{round}(\Delta^{r}_t),\ 0,\ \bar q^{r}\bigr),
    \qquad \Delta^{r}_t \;=\; 0 \text{ at zero parameters},
\end{equation}
where $q^{\mathrm{gate},r}_t$ is the pairwise base-stock gate's order on relation $r$ and $\Delta^{r}_t$
is the soft-tree residual (identity leaf, signed). Because the residual vanishes at zero parameters,
\emph{generation~0 reproduces the gate byte-exact} (proven by an in-crate test), and \emph{every}
optimizer seed is therefore anchored at the gate by construction; CMA-ES then searches outward in the
residual space only. This restores the gate-reproducing warm-start floor that the linear-leaf flow head
...
$\approx40\times$). On the mixed distribution-and-assembly network, the residual gate-backbone head of
Section~\ref{sec:pirhoo-policy} (Eq.~\eqref{eq:residual-head}) upgrades the row from a gate-match to a
\emph{robust gate beat}. The earlier linear-leaf flow head was not gate-invertible on this topology, so
its eight CMA seeds scattered to a seed-mean $306.10\pm22.89$ ($+2.82\%$ above the $297.69$ gate, only
four of eight seeds below it) --- parity, not a beat. With the residual head, which anchors every seed
at the gate by construction, the structure found by a search over the decoder classes (a
...
optimizer seeds ($-2.20\%$ against the $297.69$ gate; all five seeds below it, per-seed
$287.36,294.42,290.74,293.22,289.93$). The seed scatter collapses from $\pm22.89$ to $2.49$
($\approx0.86\%$ of the mean) precisely because the residual head reproduces the gate exactly at
...
\midrule
Best pairwise base-stock (gate, order-up-to $[8,7,9]$) & $60.24$ & --- & --- \\
...
\midrule
\multicolumn{4}{l}{\emph{Mixed distribution-and-assembly network} (gate $297.69$, echelon order-up-to $[36,13,7]$)} \\
Best pairwise base-stock (gate)                & $297.69\pm0.67$ & --- & --- \\
Flow-head linear leaf (seed mean, $N=8$)       & $306.10\pm22.89_{\rm seed}$ & $+2.82\%$ & gate-match \\
Residual-head soft tree (seed mean, $N=5$)     & $\mathbf{291.136\pm2.49_{\rm seed}}$ & $\mathbf{-2.20\%}$ & robust beat \\
\midrule
\multicolumn{4}{l}{\emph{Pure assembly network} (gate $283.34$, echelon order-up-to $[52,52,52]$)} \\
...
mean above the gate, $4/8$ seeds below it, scatter $\pm22.89$); the residual gate-backbone head of
Eq.~\eqref{eq:residual-head} anchors every seed at the gate by construction, collapsing the scatter to
$\pm2.49$ and turning the row into a robust gate beat ($5/5$ seeds below the gate, mean${}+{}$std
...
gate rows; the production/assembly/distribution network's own serial, pure-assembly, \emph{and}
mixed-topology pairwise base-stock gates). We \emph{match}, and say so explicitly, when the comparator is provably optimal
...
win. We also report \emph{gate-matches} when seed variation does not support a learned beat, as on the
...
strongly heterogeneous $K{=}10$ at $+12.44\%$ over five seeds) are over the tuned same-protocol heuristic
gate, not over PPO --- which is cross-protocol context only. (iv)~The
...
gate-match to a seed-robust $-2.20\%$ beat by the residual gate-backbone head), but the environment has

...
general-network backorder, and serial multi-echelon); the final subsection
(Section~\ref{app:additional-env-validation}) records the same fidelity checks for further
environments in our suite that we validate against the literature. The three remaining studied
...
reported strictly as a bound), and the production/assembly/distribution network is a faithful but
not literature-verified environment whose only published anchor --- the single-node newsvendor cost
--- is recorded in Section~\ref{app:additional-env-validation}.
...
The per-problem subsections above complete the published-number literature validation for the seven
literature-anchored problems studied in this paper. For completeness we record here a separate,
smaller bucket: two further inventory
environments that we implement and validate against the literature, but on which we
do \emph{not} train policies or report learned-policy results in this paper. We note them only because
they bear on reproducibility --- our environments reproduce published benchmarks beyond the
problems we report results for. Table~\ref{tab:additional-env-validation} covers
a joint-replenishment system \citep{vanvuchelen2020jrp} and a production/assembly/distribution network
\citep{pirhooshyaran2021scn}; each reproduces its published benchmark (a published optimal action and
an exact newsvendor cost, respectively) to within a tight tolerance, and we report them only as
environment-fidelity checks, not as results.

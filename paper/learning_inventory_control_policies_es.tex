...
function approximators (linear maps, one-hidden-layer networks, and soft decision trees) for seven
problems: lost sales, fixed-ordering-cost lost sales, dual sourcing, divergent multi-echelon control
with special delivery, perishable inventory, general-network backorder, and serial (Clark--Scarf)
multi-echelon control. A recurring design lever is that the \emph{action parameterization is
...
three reasons that hold across all seven problems we study. First, it optimizes the
...
\begin{enumerate}
    ...
    policies are competitive across seven inventory problems --- lost sales, fixed-cost lost sales, dual
    sourcing, divergent and serial multi-echelon, general-network backorder, and perishable inventory:
    ...
    $\approx 14.4\%$ on both multi-echelon settings; on the three further problems a soft tree beats the
    best base-stock gate for perishable inventory, beats the published benchmark of a general-network
    backorder system by over $20\%$, and matches the proven serial Clark--Scarf optimum.
...
Sections~\ref{sec:lostsales}--\ref{sec:serial} then treat each problem as a self-contained
unit --- lost sales and fixed-cost lost sales, dual sourcing, divergent multi-echelon control,
perishable inventory, general-network backorder, and serial multi-echelon control --- each stating
...
then developed in its own self-contained section (Sections~\ref{sec:lostsales}--\ref{sec:serial}).
...
the realized objective directly, so the loop is unchanged across our seven problems.
...
\caption{CMA-ES policy optimization (used for all seven problems)}
...
Synthesizing across the seven problems, three themes emerge. First, a \emph{single, generic learning
...
seven inventory problems: lost sales, fixed-cost lost sales, dual sourcing, divergent and serial
multi-echelon, general-network backorder, and perishable inventory. Relative to DRL, CMA-ES needs few hyperparameters, supplies its own
...
validate all seven problems studied in this paper (vanilla lost sales, fixed-cost lost sales,
multi-echelon Van~Roy / dual sourcing, perishable inventory, general-network backorder, and serial
multi-echelon); the final subsection (Section~\ref{app:additional-env-validation}) records the same
fidelity checks for two additional environments in our suite that we validate against the literature
but do not train on or report learned-policy results for here.
...
The per-problem subsections above complete the literature validation for all seven problems studied in
this paper. For completeness we record here a separate, smaller bucket: two further inventory

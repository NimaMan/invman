...
($-2.20\%$, five-seed mean, all seeds below the gate). The hardest-instance structures (a constant-leaf
multi-retailer geometry and the residual gate-backbone head) were found by an automated, agentic
policy-structure search scored by the same seed-robust gate.
...
\begin{enumerate}
    ...
    gate-match to a seed-robust $-2.20\%$ beat. The two hardest-instance structures were surfaced by
    an automated, agentic policy-structure search wrapping the same CMA-ES loop.
...
the floor is the \emph{leaf class}: an automated structure search (Section~\ref{sec:owmr-agentic})
...
anchor) --- is now also a robust gate beat. An automated policy-structure search
(Section~\ref{sec:owmr-agentic}) discovered that the decisive change here is the \emph{leaf class}: a
...
constant-leaf geometry from the automated search. The
...
oblique-split, per-retailer geometry surfaced by the automated structure search of
Section~\ref{sec:owmr-agentic} ($N=10$ seeds, std $\approx0.76\%$ of the mean); the linear-leaf
...
high-CV ten-retailer instance, and --- via the automated search's constant-leaf geometry --- the
...
\subsection{How the winning structure was found: agentic policy-structure search}
\label{sec:owmr-agentic}

The constant-leaf geometry that turned the hardest heterogeneous $K{=}10$ row from a tie into a
$+12.57\%$ gate beat was not hand-chosen --- it was discovered by an automated search over policy
\emph{structures} that wraps the same CMA-ES loop used throughout this paper. The search is a small
outer evolutionary loop in which a large-language-model agent proposes a policy-structure
\emph{specification} in a compact domain-specific language (action head, per-dimension geometry,
leaf class, split type, depth, feature basis, and warm start); each proposal is compiled to a soft-tree
policy, warm-started at the gate-invertible anchor so generation~0 reproduces the gate exactly, trained
by the inner CMA-ES on the Rust rollout oracle, and then scored under the seed-robust gate protocol of
Section~\ref{sec:methods-eval}. A specification is kept only if it is a \emph{robust gate beat} ---
every one of $\ge5$ optimizer seeds below the gate \emph{and} mean${}+{}$std below the gate ---
otherwise it is logged as parity, never a win; the deployed cost is always the better of the trained
policy and the gate floor. Quality--diversity pressure (best-per-niche elite contexts plus an
anti-repeat rule that forbids re-proposing an already-tried structural signature) keeps the proposer
from collapsing onto the single archived best. This is what surfaced the \emph{constant}-leaf head ---
a structure the linear-leaf hand-search had never tried, and which a short, fixated run also missed
until the diversity pressure was added. The same machinery, extended with a residual action head,
produces the mixed-network result of Section~\ref{sec:pirhoo-results}. The contribution of this paper
remains the \emph{learned policies and their action geometries}; the search is simply the mechanism
that automates the one-structure-per-round human step, under the same honest-verdict discipline.
...
at the gate by construction, the structure discovered by the automated policy-structure search of
Section~\ref{sec:owmr-agentic} (a residual-base-stock head with linear leaves, oblique splits, and a
per-relation geometry) improves the per-period cost to a seed-robust mean $291.136\pm2.49$ over five

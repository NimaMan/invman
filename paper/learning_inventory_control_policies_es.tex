...
path-probabilities $\pi_\ell(x)$: each internal node $n$ routes left with probability
$\sigma(w_n^\top x + b_n)$ and right otherwise, and $\pi_\ell(x)$ is the product of the routing
...
Training uses CMA-ES with population $64$ and a short training horizon of $2000$ periods (the
multi-echelon study additionally sweeps the action design against tree depth). Evaluation reports the
long-run average cost over a long simulated horizon with a warm-up discard, averaged over multiple
seeds: for \emph{lost sales} and \emph{fixed-cost lost sales}, a horizon of $10^6$ periods averaged
across $10$ consecutive seeds; for \emph{multi-echelon}, a horizon of $3\times10^4$ periods and $6$
seeds. For \emph{dual sourcing} we additionally use a common-random-number (CRN) protocol --- the
same $70$ demand-path seeds and horizon $6\times10^4$ for every policy --- so the dominant per-seed
variance cancels and the paired cost difference is statistically meaningful (paired SEM
$\sim0.002$--$0.006$ versus per-seed standard deviation $\sim0.5$). Each problem's reported
single-state actions are independently validated against a Rust rollout, with the cross-check detailed
...
independent Rust policy forwards used for the cross-checks are also exposed.}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[x=2.1cm,y=1.25cm]
    ...
    \node[above=10,align=center,myblue!60!black]  at (N2-1.90) {hidden layer $H$\\[-0.2em]\footnotesize (NN backbone only)};
...
Rust policy forward by construction.}
\label{tab:worked}
\begin{tabular}{lcc}
\toprule
...
independent Rust policy forward at that state by construction.
...
All six lost-sales policies were validated by a full Python-versus-Rust rollout cost match on a fixed
...
soft-tree single-state actions equal the independent Rust policy forward by construction, and the
Python tree forward agrees with the Rust forward at each reported state.
...
by a full Python-versus-Rust rollout cost match on a fixed Uniform$\{0,\dots,4\}$ demand sequence of
...
control and matching it against the independent Rust scalar forward over $5000$ random raw states
...
\ref{sec:dual-policy}, and~\ref{sec:me-policy}) and their Rust cross-checks show the mapping from state
...
$h=1$, $p=4$, $L=4$, and $D_t\sim\mathrm{Poisson}(5)$ (repository anchor
\texttt{lit\_poisson\_p4\_l4}). Table~\ref{tab:vanilla-lost-sales-validation} reports the reference
costs and a Rust rollout reproduction for the heuristic policies (horizon $10^5$, seed $123$). This is
...
$L=2$, $p=14$, $K=5$, $h=1$, $\mu=5$ (the repository anchor
\texttt{bijvank2015\_table1\_l2\_p14\_k5} in the fixed-order-cost reference catalog). The

...
%       committed at 222d46f). FIFO learned -1457.9032, best base-stock -1475.0825
%       (+1.16%, paired SEM 1.815 -> 9.5x), LIFO learned -1553.1608,
%       best base-stock -1565.9833 (+0.82%, paired SEM 1.949 -> 6.6x). VI optima
...
$-1.1\%$, $3$ seeds $\times\,500$ replications); this reproduction is the keep/discard comparator. The paper
...
The keep/discard floor is the heuristic-reproducing CMA-ES warm start (Section~\ref{sec:methods-cmaes}). We
...
geometry is not a free win --- at screening it \emph{loses} to the best heuristic by $-12.79\%$ on the
...
published PPO row is reported as the strongest \emph{learned} reference, not as the keep/discard comparator.

\paragraph{Evaluation protocol.}
Policies are trained with warm-started CMA-ES (population $32$, full-budget campaigns of
$400$--$800$ generations) and evaluated under the common-random-number protocol of
...
block under the deployed allocation. A same-seed win is claimed only when the paired-difference
advantage exceeds its standard error; a headline configuration is claimed only when a small
independent CMA-seed block has the same sign. The deployed policy is the better of the trained CMA-ES
incumbent and the heuristic-reproducing warm-start anchor.
...
same-protocol best heuristic $91890.25$ to a $5$-seed mean $85395\pm1031$ ($+7.07\%$ against the best heuristic; all six
seeds beat it), so this row is now a seed-robust beat over the best heuristic. The hardest, strongly heterogeneous
$K{=}10$ instance --- which an earlier hand-search could only \emph{tie} (the trained incumbent landed
above the strong asymmetric heuristic within budget, so the deployed policy fell back to the heuristic-reproducing
...
\emph{constant}-leaf, oblique-split, per-retailer depth-2 \texttt{echelon\_targets} tree improves the
...
linear-leaf hand-search never tried --- lets CMA-ES realize the improvement within budget.
...
deviation over $6$ independent CMA seeds (all $6$ beat the best heuristic); the third row is the deployed
warm-start floor. The published PPO column is the strongest \emph{learned} reference of
\citet{kaynov2024owmr}, reported as cross-protocol context rather than a keep/discard comparator.}
...
hand-search only tied this row.
...
On all three the learned per-retailer soft tree \emph{matches} (ties) the best tuned in-repo
heuristic to within numerical error rather than beating it: with the per-retailer warm-start
floor the trained CMA-ES incumbent lands above the strong heuristic on the held-out block, so
the deployed policy falls back to the heuristic-reproducing anchor (paired advantage $0.00$, not
...
strongest in-repo heuristic exactly. The promoted seed-robust \emph{beats} in
...
three the learned policy ties the best heuristic (deployed policy $=$ heuristic-reproducing warm-start
...
echelon-targets geometry under min-shortage allocation. The trained CMA-ES
incumbent's held-out cost was above the best heuristic on every instance ($1793.11$, $1584.47$,
$1802.12$ respectively), so the deployed policy is the heuristic-reproducing warm-start anchor and
...
as an achievable target. (ii)~The like-for-like keep/discard \emph{heuristic} is the best tuned order-up-to
...
tuned order-up-to heuristic at full budget. On both instances the learned policy \emph{beats the best heuristic by a
...
\emph{generation~0 reproduces the best heuristic byte-exact} (proven by an in-crate test), and \emph{every}
...
lead-time or network structure, strongest in-repo heuristic, exact optimum or bound where available,

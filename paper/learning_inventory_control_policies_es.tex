...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[x=1cm,y=1cm]
    ...
    \draw[matflow] (sup.south) to[bend right=22] (wh.west);
...
Section~\ref{sec:pirhoo} result (Section~\ref{app:pirhoo-validation}). The final subsection
(Section~\ref{app:additional-env-validation}) records the same kind of fidelity check for the one
further environment in our suite that we validate against the literature but do \emph{not} train or
report a learned policy on in this paper. The two remaining studied
...
\subsection{Additional verified environment (not studied in this paper's experiments)}
\label{app:additional-env-validation}

The per-problem subsections above complete the literature validation for the studied problems,
including the single-node fidelity check for the production/assembly/distribution network. For
completeness we record here the one further inventory environment that we implement and validate
against the literature but on which we do \emph{not} train a policy or report a learned-policy result
in this paper: the joint-replenishment system of \citet{vanvuchelen2020jrp}. We note it only because
it bears on reproducibility --- our environment reproduces a published benchmark beyond the problems
we train on.

Before treating the joint-replenishment environment as faithful, we validate it against the one exact,
executable quantity that \citet{vanvuchelen2020jrp} state in prose. Their paper reports per-setting
optimality gaps only as a figure (Figure~2), so there is no published absolute optimal-cost table to
match; the single exact result they state is an \emph{optimal action}. For their small-scale
setting~5 (two items, truck capacity $V=6$, major order cost $K=75$, minor order costs $k=[40,10]$,
holding $h=[1,1]$, penalty $b=[19,19]$, demand $U[0,5]$ and $U[0,3]$, discount $\gamma=0.99$), they
state that in state $(I_1,I_2)=(5,0)$ the optimal policy ships exactly one full truckload to item~2 ---
the action $q=(0,6)$ --- whereas both of their heuristics instead order $q=(2,4)$
\citep{vanvuchelen2020jrp}. Running infinite-horizon value iteration over the environment's \emph{own}
one-period cost and transition reproduces this published optimal action exactly ($q=(0,6)$ at state
$(5,0)$), and the same value iteration confirms it is strictly cheaper than the heuristic action
$q=(2,4)$. As a hand-checkable cost anchor, the environment evaluates the published optimal action at a
worked demand $d=(2,4)$ to a one-period cost of $90$ (major $75$ $+$ minor $10$ on the single ordered
item, plus holding $5$ on the resulting inventory $(3,2)$), matching the paper's cost convention.

\begin{table}[!htbp]
\centering
\caption{Reproduction of the \citet{vanvuchelen2020jrp} Figure~3 joint-replenishment anchor
(small-scale setting~5). The environment reproduces the published optimal action by independent
value iteration over its own dynamics, and evaluates that action to the published cost convention at a
worked demand. \citet{vanvuchelen2020jrp} report no absolute optimal-cost table (per-setting gaps
appear only as a figure), so the published anchor is an optimal action rather than a cost.}
\label{tab:additional-env-validation}
\begin{tabular}{lcc}
\toprule
Quantity & Published & Reproduced \\
\midrule
Optimal action at state $(5,0)$ & $(0,6)$ & $(0,6)$ (env value iteration) \\
Heuristic action at state $(5,0)$ & $(2,4)$ & $(2,4)$ (strictly costlier) \\
One-period cost at $q=(0,6)$, $d=(2,4)$ & --- & $90$ (env cost convention) \\
\bottomrule
\end{tabular}
\end{table}
\FloatBarrier

This is a verified environment we deliberately do \emph{not} study as a learned-policy result in this
paper: we report no dedicated problem section, no training, and no learned-versus-heuristic comparison
for joint replenishment. It appears here solely as an environment-fidelity check, confirming that our
implementation reproduces the published Vanvuchelen et al.\ optimal action under its own dynamics.

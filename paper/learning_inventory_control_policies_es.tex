...
gate on the asymmetric three-retailer ($+4.63\%$), high-CV ten-retailer ($+7.16\%$), and the
previously-unsolved strongly heterogeneous ten-retailer ($+12.57\%$, ten-seed mean, all seeds below
...
\begin{enumerate}
    ...
    ($+12.57\%$, ten-seed mean) while not claiming a robust PPO beat; on ameliorating inventory it raises long-run
...
Learned soft tree, $5$-seed mean (ours) & $\mathbf{7{,}837.0 \pm 189.7_{\rm seed}}$ & $\mathbf{-24.3\%\pm1.8\%}$ \\
\addlinespace[2pt]
...
Learned soft tree, $5$-seed mean (ours) & $\mathbf{2{,}484.6 \pm 11.3_{\rm seed}}$ & $\mathbf{-36.8\%\pm0.3\%}$ \\
\addlinespace[2pt]
\multicolumn{3}{l}{\footnotesize Cross-protocol context (not head-to-head):} \\
...
gate beat ($6$-seed mean, all six seeds below the gate). A raw per-retailer order geometry without a gate-reproducing anchor loses by $-50.84\%$,
...
geometry --- is what robustly beats the gate there, turning a long-standing tie into a $+12.57\%$
...
$1115.44\pm5.51$ across six independent CMA seeds ($+4.63\%$ against the gate; all six seeds beat
...
same-protocol gate $91890.25$ to a $6$-seed mean $85310\pm946$ ($+7.16\%$ against the gate; all six
...
$100$-period cost from the same-protocol gate $50445.20$ to a $10$-seed mean $44105.01\pm337.3$
($+12.57\%$ against the gate; all ten seeds below it, $\mathrm{std}\approx0.76\%$ of the mean). The
...
($+4.63\%$, all six seeds), the high-CV $K{=}10$ instance ($+7.16\%$, all six seeds), and the
strongly heterogeneous $K{=}10$ instance ($+12.57\%$, all ten seeds), the last via the
constant-leaf geometry found by the decoder-class search. The
published PPO scalar is reported only as cross-protocol context: on $K{=}3$ the six-seed costs
straddle the PPO line (the seed-mean is $+0.31\%$ below PPO and five of six seeds fall below it, but
the seed spread exceeds the mean margin, so the beat is not robust), and on the $K{=}10$ rows PPO
remains cheaper (the high-CV seed-mean is $\approx 7\%$ above PPO, $0/6$ below; on the heterogeneous
row the $44105.01$ seed-mean sits $\approx 3\%$ above the published PPO scalar $42835.02$). We
...
\midrule
heterogeneous-$3$ & depth-2 seed mean ($N=6$) & $\mathbf{1115.44\pm5.51_{\rm seed}}$ & $1169.59$ & $\mathbf{+4.63\%}$ & $1118.92$ \\
high-CV symmetric-$10$ & depth-3 seed mean ($N=6$) & $\mathbf{85310\pm946_{\rm seed}}$ & $91890.25$ & $\mathbf{+7.16\%}$ & $79727.39$ \\
heterogeneous-$10$ & const-leaf seed mean ($N=10$) & $\mathbf{44105.01\pm337.3_{\rm seed}}$ & $50445.20$ & $\mathbf{+12.57\%}$ & $42835.02$ \\
\bottomrule
\end{tabular}
...
on each (heterogeneous-$3$ mean$-$std $=+4.16\%$, high-CV mean$-$std $=+6.13\%$, and heterogeneous-$10$
mean$-$std $=+11.90\%$ still clear it). The heterogeneous-$10$ row uses the \emph{constant}-leaf,
oblique-split, per-retailer geometry found by a search over the decoder classes
($N=10$ seeds, std $\approx0.76\%$ of the mean); the linear-leaf
...
is $\approx 7\%$ above PPO ($0/6$ below), and on the heterogeneous ten-retailer row the seed-mean
$44105.01$ sits $\approx 3\%$ above the published PPO scalar $42835.02$, so PPO remains cheaper there.
\end{table}
\FloatBarrier
...
$\approx 7\%$ above PPO ($0/6$ seeds below), and the strongly heterogeneous ten-retailer seed-mean is
$\approx 3\%$ above PPO; our wins on that problem (asymmetric $K{=}3$, high-CV $K{=}10$, and the
strongly heterogeneous $K{=}10$ at $+12.57\%$ over ten seeds) are over the tuned same-protocol heuristic

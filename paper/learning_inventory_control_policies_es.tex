...
gate on the asymmetric three-retailer ($+4.63\%$) and high-CV ten-retailer ($+7.16\%$) instances
(six-seed means, all seeds below the gate) but does not robustly beat the published PPO scalar;
the ameliorating-inventory result beats a purchase-only gate while remaining a large gap-to-bound;
and the production/assembly/distribution network is reported as learned-versus-own-heuristic
research, with the mixed topology now treated as a gate-match rather than a beat.
...
\begin{enumerate}
    ...
    multi-retailer it gives seed-robust same-protocol gate beats on the asymmetric three-retailer and
    high-CV ten-retailer instances while not claiming a robust PPO beat; on ameliorating inventory it raises long-run
    ...
    the environment's own pairwise base-stock on the serial and pure-assembly topologies while the
    mixed topology is a seed-noisy gate-match.
...
seeds beat it), so this row is now a seed-robust gate beat. On the hardest, strongly heterogeneous
$K{=}10$ instance the deployed policy still ties the gate: with the warm-start floor the trained
incumbent lands above the strong asymmetric gate within budget, so the deployed policy falls back to
the gate-reproducing anchor. This tie is \emph{search-limited}, not representation-limited --- the
gate lies inside the learned policy's class, but CMA-ES does not improve on it over the larger target
space within the current budget.

\paragraph{Interpretation.}
The honest headline is therefore narrower than a best-seed story and stronger than the old plateau:
the learned soft tree \emph{robustly beats the strongest same-protocol tuned heuristic gate} on both
the asymmetric $K{=}3$ partial-backorder instance ($+4.63\%$, all six seeds) and the high-CV
$K{=}10$ instance ($+7.16\%$, all six seeds), and ties the hardest heterogeneous $K{=}10$ row. The
...
remains cheaper (the high-CV seed-mean is $\approx 7\%$ above PPO, $0/6$ below). We therefore do
\emph{not} claim a robust PPO beat on any row. The mechanism is the generalized warm-start floor plus
the adequately batched, moderate-variance training protocol of Section~\ref{sec:owmr-policy}; depth-3
...
heterogeneous-$10$ & deployed warm-start floor & $50445.20$ & $50445.20$ & $+0.00\%$ & $42835.02$ \\
\bottomrule
\end{tabular}
\par\medskip\footnotesize The comparator is the strongest tuned heuristic gate (itself stronger than
the published base-stock). Both learned rows are seed-robust gate beats: all $6$ seeds beat the gate
on each (heterogeneous-$3$ mean$-$std $=+4.16\%$, high-CV mean$-$std $=+6.13\%$ still clear it).
...
is $\approx 7\%$ above PPO ($0/6$ below), and PPO remains cheaper on the heterogeneous ten-retailer
row.
\end{table}
\FloatBarrier
...
high-CV ten-retailer instance, both now reported as $6$-seed means with every seed below the gate.
...
$\approx40\times$). On the mixed distribution-and-assembly network, the later seed-robust audit
corrects the earlier best-of-three draft: at the paper's actual flow-$10$ warm start, eight
independent CMA seeds have seed-mean $306.10\pm22.89$, which is $+2.82\%$ above the $297.69$ gate,
with four of eight seeds below the gate. The right verdict is therefore parity / gate-match, not a
robust beat. As for the serial case, these comparators are the \emph{environment's own} grid-searched
...
benchmarks) --- so these are research learned-versus-own-heuristic results across serial, mixed
distribution-plus-assembly, and pure-assembly structures, and are \emph{not} published-number beats.
...
Learned soft tree, depth $2$ (seed mean, $N=8$) & $306.10\pm22.89_{\rm seed}$ & $+2.82\%$ & gate-match \\
\midrule
...
comparison, not a published-number beat. The serial case-3 and pure-assembly rows are single-/two-seed
results (a multi-seed audit is pending); only the mixed-network row was re-run as a seed-robust
aggregate. The mixed row is reported as seed-noisy parity: the seed mean
is above the gate, but $4/8$ seeds fall below it, so an honest deployment floor treats the row as a
gate-match rather than a beat. The serial optimum $47.65$ (Section~\ref{sec:serial}) is structurally
...
seed-robust asymmetric three-retailer gate row; the production/assembly/distribution network's own
serial and pure-assembly pairwise base-stock gates). We \emph{match}, and say so explicitly, when the comparator is provably optimal
...
win. We also report \emph{gate-matches} when seed variation does not support a learned beat, as in the
mixed production/assembly topology. And we report a \emph{gap to a bound} without ever calling it ``beaten'': the ameliorating-inventory
...
$\approx 7\%$ above PPO ($0/6$ seeds below), and PPO remains cheaper on the hardest row; our wins on
that problem (asymmetric $K{=}3$ and high-CV $K{=}10$, both $6$-seed gate beats) are over the tuned
same-protocol heuristic gate, not over PPO. (iv)~The
...
own best heuristic on the serial and pure-assembly topologies, while the mixed topology is a seed-noisy
gate-match, and none of these is a published cost. (vi)~Methodologically, CMA-ES --- though far more

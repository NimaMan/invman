...
\caption{Soft-gated direct decoder. The latent outputs are split into an order gate and a positive quantity head, then multiplied and rounded to produce the integer order.}
...
\caption{Soft-gated ordinal decoder. The gate controls whether to order, while the ordinal threshold probabilities sum to a soft count of units.}
...
\centering
\SoftTreeFigure
\caption{Soft-tree decoder with depth 2. The action is the rounded mixture of local leaf quantities, weighted by soft routing probabilities.}
...
\caption{Categorical score-per-order baseline. The latent outputs score each feasible order size directly, and the policy selects the order with the largest score.}

...
Figures~\ref{fig:env-lostsales} and~\ref{fig:env-fixedcost} depict the vanilla and fixed-ordering-cost variants as event-based interactions. Both systems share the same physical flow of order requests, lead-time arrivals, demand, and lost demand; the fixed-cost variant additionally charges a setup cost whenever the order request is positive.

\begin{figure}[H]
\centering
\LostSalesEventFigure
\caption{Vanilla lost-sales environment as an event-based interaction. A dashed arrow represents the order request sent to the supplier; a solid arrow represents the lead-time shipment arriving from a previous request.}
\label{fig:env-lostsales}
\end{figure}

\begin{figure}[H]
\centering
\FixedCostLostSalesEventFigure
\caption{Fixed-ordering-cost lost-sales environment as an event-based interaction. The physical event sequence is unchanged relative to Figure~\ref{fig:env-lostsales}; the setup cost is charged when $q_t>0$.}

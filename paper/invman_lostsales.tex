...
The decoder is the main policy-design choice. We first record the earlier categorical score-per-order representation because it is the easiest action parameterization to visualize: the policy scores a finite menu of order sizes and selects one. We then use three decoder families in the reported new results: soft-gated direct quantities, soft-gated ordinal quantities, and soft trees with linear leaves.
...
\caption{Lost-sales event sequence and cost accounting for one period. The vanilla model sets $K=0$; the fixed-ordering-cost model adds the setup term when the order is positive.}

...
\input{figures/policy_architecture_inline}
...
The remainder of the paper is organized as follows. Section~\ref{sec:literature} reviews learned inventory policies, evolution strategies, and lost-sales heuristics. Section~\ref{sec:policy_representation} describes the policy interface and CMA-ES optimization loop. Section~\ref{sec:lost_sales} formulates the vanilla and fixed-cost lost-sales problems. Section~\ref{sec:experiments} reports the benchmark setup and results. Section~\ref{sec:managerial_insights} discusses practical implications, and Section~\ref{sec:conclusions} concludes.
...
DRL is regarded as a promising research direction for inventory applications, and a range of Reinforcement Learning algorithms has been successfully applied to a variety of problems. Deep Q Networks (DQN) is amongst the earliest DRL algorithms, and is successfully applied to the beer game \citep{oroojlooyjadid2022deep} and to perishable inventory control \citep{deMoor2022perishable}. The flexibility of DRL algorithms to adapt to different problems was convincingly demonstrated with the application of A3C to dual sourcing, lost sales, and one-warehouse multi-retailer problems \citep{gijsbrechts2022drl}. Recent work is increasingly adopting proximal policy optimization (PPO), for example for the joint ordering problem \citep{vanvuchelen2020jrp}, the asymmetric one-warehouse multi-retailer problem \citep{kaynov2024owmr}, stochastic capacitated lot sizing \citep{vanhezewijk2023using}, and inventory control for non-stationary stochastic demand \citep{dehaybe2024deep}. \cite{temizoz2025dcl} propose a different algorithm that is specifically designed for highly stochastic models, viz. Deep Controlled Learning (DCL), and demonstrate superior results for several classical inventory models \citep[see also][]{van2025scalable}. DQN, A3C, PPO, and DCL, although differing in their specific mechanisms, all utilize state-action-reward sequences as the foundational structure for learning and optimizing policies in Reinforcement Learning environments. That is, these algorithms involve the attribution of rewards obtained over some simulation trajectory to individual actions taken in that trajectory, and subsequently the tuning of neural network parameters to promote the use of actions that are deemed to output favorable actions.
...
We use two backbone classes and one tree mechanism. A linear backbone is a single affine map $u_\theta(\mathbf x)=W\mathbf x+b$, as shown in Figure~\ref{fig:linear_policy}. A neural-network backbone is a one-hidden-layer network $u_\theta(\mathbf x)=V\phi(U\mathbf x+c)+e$ with hidden width $H=8$ and SELU activation $\phi$ \citep{Klambauer2017Self}, as shown in Figure~\ref{fig:neural_policy}. A soft decision tree routes the normalized state through logistic oblique splits and returns a path-probability mixture of leaf quantities.

\begin{figure}[H]
\centering
\LinearPolicyFigure
\caption{Linear policy backbone. The normalized inventory state is mapped directly to latent policy outputs through a single affine transformation.}
\label{fig:linear_policy}
\end{figure}

\begin{figure}[H]
\centering
\NeuralPolicyFigure
\caption{Neural-network policy backbone. The normalized inventory state passes through one hidden layer with $H=8$ units before producing latent policy outputs.}
\label{fig:neural_policy}
\end{figure}

\subsection{Decoder heads}

The decoder is the main policy-design choice. We use three decoder families in the reported new results: soft-gated direct quantities, soft-gated ordinal quantities, and soft trees with linear leaves. We also record the earlier categorical score-per-order representation as a baseline action parameterization.
...
\begin{figure}[H]
\centering
\SoftGatedDirectDecoderFigure
\caption{Soft-gated direct decoder. The latent outputs define an order switch and a nonnegative continuous quantity; their product is rounded and projected onto the feasible integer action set.}
\label{fig:decoder-direct}
\end{figure}
...
\begin{figure}[H]
\centering
\SoftGatedOrdinalDecoderFigure
\caption{Soft-gated ordinal decoder. Ordinal threshold probabilities act like fractional unit indicators; their sum is gated and rounded to obtain the integer order.}
\label{fig:decoder-ordinal}
\end{figure}
...
\]
where $\pi_\ell(\mathbf x)$ is the routing path probability. Figure~\ref{fig:decoder-tree} illustrates the depth-2 case.

\begin{figure}[H]
\centering
\SoftTreeFigure
\caption{Soft-tree decoder with depth 2. Each leaf proposes a quantity, the soft path probabilities form a weighted vote, and the resulting continuous order is rounded.}
\label{fig:decoder-tree}
\end{figure}

\paragraph{Categorical score-per-order baseline}
The earlier score-per-order representation emits one score $y_q$ for each feasible order $q\in\{0,\ldots,\bar q\}$ and chooses the largest score:
\[
q_t=\argmax_{q\in\{0,\ldots,\bar q\}} y_q.
\]
This decoder fixes the action grid in the output layer rather than learning an order/no-order gate.

\begin{figure}[H]
\centering
\CategoricalDecoderFigure
\caption{Categorical score-per-order baseline. The policy treats feasible order sizes as a finite menu, assigns one score to each option, and selects the highest-scoring order.}
\label{fig:decoder-categorical}
\end{figure}
...
Figure~\ref{fig:env-lostsales} depicts the vanilla and fixed-ordering-cost variants as one event-based interaction. Both systems share the same physical flow of order requests, lead-time arrivals, demand, and lost demand; the fixed-cost variant additionally charges a setup cost whenever the order request is positive, while the vanilla case is recovered by setting $K=0$.

\begin{figure}[H]
\centering
\LostSalesEventFigure
\caption{Lost-sales environment as an event-based interaction for the vanilla and fixed-ordering-cost settings. A dashed green arrow represents the order request sent to the supplier; a solid blue arrow represents the lead-time shipment arriving from a previous request. The setup term is zero in the vanilla case and active when $K>0$ and $q_t>0$.}

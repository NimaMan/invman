...
\begin{tikzpicture}[x=1cm,y=1cm]
  \node[invinput,minimum width=24mm] (state) at (0,0)
    {\textbf{state}\\$\mathbf S_t\in\mathbb Z_{\ge0}^{L}$};
  \node[invmiddle,minimum width=31mm] (norm) at (3.0,0)
    {normalize\\$\mathbf x=\mathbf S_t/\kappa$};
  \node[invmiddle,minimum width=36mm] (backbone) at (6.8,0)
    {backbone\\linear, NN, or tree};
  \node[invdecoder,minimum width=34mm] (decoder) at (10.4,0)
    {decoder\\direct, ordinal, or tree};
  \node[invoutput,minimum width=25mm] (action) at (13.7,0)
    {\textbf{order}\\$q_t\in\mathbb Z_{\ge0}$};
  ...
  \end{scope}
  \node[invblue,below=2pt of policy.south]
    {\footnotesize policy $\pi_{\btheta}$, with parameters $\btheta$ optimized by CMA-ES};
  \node[invgreen,above=2pt of state.north] {\footnotesize observed};
  \node[invred,above=2pt of action.north] {\footnotesize feasible action};
\end{tikzpicture}}%
}

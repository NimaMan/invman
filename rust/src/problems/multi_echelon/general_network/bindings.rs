...
use crate::problems::multi_echelon::general_network::demand::{parse_demand_distribution_kind, DemandModel};
use crate::problems::multi_echelon::general_network::env::{
    NetworkEdge, NetworkInventoryGraph, NetworkNodeMode,
};
use crate::problems::multi_echelon::general_network::flownet::verification::verify_exact_reference_policy_performance;
use crate::problems::multi_echelon::general_network::heuristics::{
    pairwise_base_stock_requests, policy_rollout_from_paths,
};
use crate::problems::multi_echelon::general_network::rollout::{
    build_initial_state, population_rollout, rollout, rollout_from_paths,
    NetworkInventoryRolloutConfig,
};
use crate::problems::multi_echelon::general_network::verification::literature_benchmarks::literature_benchmark_summary;
...
fn general_network_soft_tree_rollout(
    flat_params: Vec<f32>,
    input_dim: usize,
    depth: usize,
...
fn general_network_soft_tree_population_rollout(
...
fn general_network_soft_tree_rollout_from_paths(
    flat_params: Vec<f32>,
    input_dim: usize,
    depth: usize,
    min_values: Vec<usize>,
...
fn general_network_policy_rollout_from_paths(
    policy_name: &str,
    params: Vec<f64>,
    num_nodes: usize,
    source_nodes: Vec<bool>,
...
#[pyfunction]
fn general_network_pairwise_base_stock_requests(
    num_nodes: usize,
    source_nodes: Vec<bool>,
    node_modes: Vec<String>,
    external_supplier_lead_times: Vec<usize>,
...
#[pyfunction]
fn general_network_flownet_policy_verification_summary(py: Python<'_>) -> PyResult<PyObject> {
...
fn general_network_literature_benchmark_summary(
    py: Python<'_>,
...
pub fn register_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(general_network_soft_tree_rollout, m)?)?;
    m.add_function(wrap_pyfunction!(
        general_network_soft_tree_population_rollout,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        general_network_soft_tree_rollout_from_paths,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        general_network_policy_rollout_from_paths,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        general_network_pairwise_base_stock_requests,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        general_network_flownet_policy_verification_summary,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        general_network_literature_benchmark_summary,
        m
    )?)?;
    Ok(())
}

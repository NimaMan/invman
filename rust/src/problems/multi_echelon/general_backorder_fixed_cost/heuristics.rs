...
fn split_by_weights(total_quantity: usize, weights: &[f64]) -> Vec<usize> {
    ...
    let mut raw = weights
        ...
        .collect::<Vec<_>>();
    let mut allocation = raw.iter().map(|value| value.floor() as usize).collect::<Vec<_>>();
    ...
    for retailer_idx in 0..network.num_retailers {
        ...
        match routing_mode {
            BenchmarkOrderRoutingMode::RandomSingleConnectionByWeight => {
                let selected_edge = sample_single_edge_by_weight(rng, &incoming_edges, &incoming_weights);
                let current_level =
                    retailer_selected_edge_inventory_position(network, state, selected_edge)?.max(0)
                        as usize;

...
fn topological_order(graph: &NetworkInventoryGraph) -> PyResult<Vec<usize>> {
    ...
    if order.len() != graph.num_nodes {
        return Err(PyValueError::new_err(
            "production_assembly_distribution_network graph must be acyclic",
        ));
    }
    Ok(order)
}
...
pub fn validate_graph(graph: &NetworkInventoryGraph) -> PyResult<()> {
    if graph.num_nodes == 0 {
        return Err(PyValueError::new_err(
            "production_assembly_distribution_network requires at least one node",
        ));
    }
    ...
    for edge in graph.edges.iter() {
        ...
        if edge.from == edge.to {
            return Err(PyValueError::new_err(
                "self-loops are not supported in production_assembly_distribution_network",
            ));
        }
    }

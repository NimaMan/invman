...
fn policy_requests(
    ...
    match policy_name {
        "pairwise_base_stock" | "node_base_stock" => {
            if params.len() != crate::problems::multi_echelon::production_assembly_distribution_network::env::supply_relation_count(graph)
            {
                return Err(PyValueError::new_err(format!(
                    "{policy_name} expects {} parameters",
                    crate::problems::multi_echelon::production_assembly_distribution_network::env::supply_relation_count(graph)
                )));
            }
            let levels = params
                .iter()

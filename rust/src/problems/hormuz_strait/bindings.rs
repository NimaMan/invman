...
use crate::problems::hormuz_strait::scenarios::{
    simulate_month_ahead_price_scenarios, HormuzDailyPriceSummary,
    HormuzMonthAheadSimulationReport, HormuzScenarioAssumption,
...
fn daily_summary_to_py(py: Python<'_>, daily: &HormuzDailyPriceSummary) -> PyResult<PyObject> {
    ...
    dict.set_item("mean_brent_price_usd_per_bbl", daily.mean_brent_price_usd_per_bbl)?;
    dict.set_item("p10_brent_price_usd_per_bbl", daily.p10_brent_price_usd_per_bbl)?;
    dict.set_item("p50_brent_price_usd_per_bbl", daily.p50_brent_price_usd_per_bbl)?;
    dict.set_item("p90_brent_price_usd_per_bbl", daily.p90_brent_price_usd_per_bbl)?;
    ...
    dict.set_item("reserve_release_million_bpd", daily.reserve_release_million_bpd)?;
    dict.set_item(
...
fn report_to_py(py: Python<'_>, report: &HormuzMonthAheadSimulationReport) -> PyResult<PyObject> {
    ...
    dict.set_item("latest_observed_close_date", &report.latest_observed_close_date)?;
    dict.set_item(
...
fn hormuz_strait_month_ahead_price_scenarios(
    ...
    Python::with_gil(|py| report_to_py(py, &simulate_month_ahead_price_scenarios(days, paths, seed)))
}

pub fn register_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(hormuz_strait_month_ahead_price_scenarios, m)?)?;
    Ok(())
}

...
pub fn validate_lost_sales_flownet() -> Result<(), Vec<FlowNetValidationIssue>> {
    validate_flownet(&canonical_lost_sales_flownet())
}

#[cfg(test)]
mod tests {
    use super::validate_lost_sales_flownet;

    #[test]
    fn canonical_lost_sales_flownet_is_valid() {
        assert!(validate_lost_sales_flownet().is_ok());
    }
}

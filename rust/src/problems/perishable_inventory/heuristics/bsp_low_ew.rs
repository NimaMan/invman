...
pub fn bsp_low_ew_order_quantity(
    ...
    let raw_order = if current_position < threshold {
        let estimated_waste =
            estimate_waste_during_lead_time(state, lead_time, demand_mean, issuing_policy);
        (low_inventory_level as f64 - current_position as f64 + estimated_waste).max(0.0)
    } else {
        (high_inventory_level as f64 - current_position as f64).max(0.0)
    };

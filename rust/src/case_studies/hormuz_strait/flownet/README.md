...
The intended use here is simulation under supplied scenarios, not embedded estimation or embedded
control optimization.

That means:

- random events can be injected from outside as ceasefire paths, closure paths, or escalation paths
- controls can be supplied from outside as reserve-release, rerouting, and allocation assumptions
- the FlowNet then tells us how the principal oil-flow dynamics evolve over time
...
- the chokepoint can experience disruption onset, persistence, reopening, or ceasefire failure
- rerouting around the Strait can introduce additional transit delay and congestion
- these stochastic paths can be provided exogenously to the model rather than estimated inside it
...
For now, these choices are supplied scenario-by-scenario rather than optimized.

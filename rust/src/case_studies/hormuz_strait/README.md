...
- estimate where oil prices can move under the current Hormuz developments
- do that conditionally on explicit disruption and control scenarios
- keep the first model focused on principal flow components rather than premature optimization
...
- FlowNet captures the principal state, flow, disruption, and reserve dynamics
- exogenous events can be supplied from outside as scenario inputs
- controls are currently supplied as scenario assumptions rather than optimized decisions
- optimization can be layered on top later without changing the simulation backbone
...
```bash
python rust/src/case_studies/hormuz_strait/scripts/fetch_and_build.py
```

Run the month-ahead scenario simulation with:

```bash
python rust/src/case_studies/hormuz_strait/scripts/run_month_ahead_simulation.py
```

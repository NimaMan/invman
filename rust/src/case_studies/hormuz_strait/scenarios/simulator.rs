...
fn daily_market_state(
    ...
    let non_hormuz_supply_response_million_bpd = (anchors
        .opec_may_2026_supply_adjustment_million_bpd
    ...
    let scarcity_premium_usd_per_bbl = MAX_SCARCITY_PREMIUM_USD_PER_BBL
        * (1.0 - (-effective_tightness_million_bpd / SCARCITY_TIGHTNESS_SCALE_MILLION_BPD).exp());
...
fn simulate_scenario(
    ...
    for (day_index, market_state) in daily_market.iter().enumerate() {
        ...
        max_effective_tightness_million_bpd =
            max_effective_tightness_million_bpd.max(market_state.effective_tightness_million_bpd);

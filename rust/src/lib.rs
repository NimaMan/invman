...
fn invman_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    problems::hormuz_strait::bindings::register_py(m)?;
    problems::joint_replenishment::bindings::register_py(m)?;
    problems::joint_pricing_inventory::bindings::register_py(m)?;
    ...
    problems::vendor_managed_inventory::bindings::register_py(m)?;
    Ok(())
}

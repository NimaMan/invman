...
pub struct LostSalesPolicyTraceRow {
    pub period: usize,
    pub demand: usize,
    pub current_inventory_before_order: i64,
    pub pipeline_before_order: Vec<usize>,
    pub raw_state: Vec<f32>,
    pub normalized_state: Vec<f32>,
    pub order_quantity: usize,
    pub arriving_order: usize,
    pub inventory_before_demand: i64,
    pub ending_inventory: i64,
    pub period_cost: f64,
    pub active_after_warmup: bool,
}
...
fn trace_from_demands_with_action<F>(
    ...
    mut action_from_state: F,
) -> PyResult<(f64, Vec<LostSalesPolicyTraceRow>)>
where
    F: FnMut(&[f32]) -> PyResult<usize>,
{
    if env_state.lead_time_orders.is_empty() {
        return Err(PyValueError::new_err("lead_time_orders must be non-empty"));
    }
    let warm_up_periods =
        warm_up_periods_for_horizon(demands.len(), warm_up_periods_ratio)?;
    let mut epoch_costs = Vec::with_capacity(demands.len());
    let mut trace = Vec::with_capacity(demands.len());

    for (period, demand) in demands.iter().enumerate() {
        let current_inventory_before_order = env_state.current_inventory;
        let pipeline_before_order = env_state.lead_time_orders.clone();
        let raw_state = build_pipeline_state(
        ...
        let normalized_state =
            normalize_pipeline_state(&raw_state, state_normalizer, state_scale)
                .map_err(PyValueError::new_err)?;
        let order_quantity = action_from_state(&normalized_state)?;

        let arriving_order = env_state.lead_time_orders.remove(0);
        env_state.lead_time_orders.push(order_quantity);
        env_state.current_inventory =
            add_arriving_order(env_state.current_inventory, arriving_order);
        let inventory_before_demand = env_state.current_inventory;
        ...
        epoch_costs.push(period_cost);
        trace.push(LostSalesPolicyTraceRow {
            period,
            demand: *demand,
            ...
            ending_inventory: env_state.current_inventory,
            period_cost,
            active_after_warmup: period >= warm_up_periods,
        });
    }

    Ok((mean_after_warmup(&epoch_costs, warm_up_periods_ratio), trace))
}
...
pub fn trace_from_demands(
    flat_params: &[f32],
    config: &LostSalesRolloutConfig,
    env_state: LostSalesState,
    demands: &[usize],
) -> PyResult<(f64, Vec<LostSalesPolicyTraceRow>)> {
    validate_config(config)?;
    if config.input_dim != env_state.lead_time_orders.len() {
    ...
    trace_from_demands_with_action(
        env_state,
        demands,
        config.state_normalizer,
        config.state_scale,
        ...
        config.fixed_order_cost,
        config.warm_up_periods_ratio,
        |state| soft_tree_action(flat_params, state, config),
    )
}
...
pub fn linear_trace_from_demands(
    flat_params: &[f32],
    config: &LostSalesLinearRolloutConfig,
    env_state: LostSalesState,
    demands: &[usize],
...
pub fn neural_trace_from_demands(
    flat_params: &[f32],
    config: &LostSalesNeuralRolloutConfig,
    env_state: LostSalesState,
    demands: &[usize],

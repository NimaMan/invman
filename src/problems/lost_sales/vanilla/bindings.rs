...
use crate::problems::lost_sales::vanilla::rollout::{
    ...
    linear_trace_from_demands as lost_sales_linear_trace_from_demands_impl,
    ...
    neural_trace_from_demands as lost_sales_neural_trace_from_demands_impl,
    ...
    trace_from_demands as lost_sales_trace_from_demands_impl, LostSalesLinearRolloutConfig,
    LostSalesNeuralRolloutConfig, LostSalesPolicyTraceRow, LostSalesRolloutConfig,
};
...
fn warm_up_periods_for_horizon(horizon: usize, warm_up_periods_ratio: f64) -> PyResult<usize> {
    if !(0.0..=1.0).contains(&warm_up_periods_ratio) {
        return Err(pyo3::exceptions::PyValueError::new_err(
            "warm_up_periods_ratio must be in [0, 1]",
        ));
    }
    Ok(((warm_up_periods_ratio * horizon as f64).floor() as usize).min(horizon))
}

fn trace_payload_to_py(
    py: Python<'_>,
    policy_name: &str,
    mean_cost: f64,
    warm_up_periods: usize,
    trace: &[LostSalesPolicyTraceRow],
) -> PyResult<Py<PyDict>> {
    let dict = PyDict::new_bound(py);
    dict.set_item("policy_name", policy_name)?;
    dict.set_item("mean_cost", mean_cost)?;
    dict.set_item("warm_up_periods", warm_up_periods)?;
    let rows = trace
        .iter()
        .map(|row| {
            let row_dict = PyDict::new_bound(py);
            row_dict.set_item("period", row.period)?;
            row_dict.set_item("demand", row.demand)?;
            row_dict.set_item(
                "current_inventory_before_order",
                row.current_inventory_before_order,
            )?;
            row_dict.set_item("pipeline_before_order", row.pipeline_before_order.clone())?;
            row_dict.set_item("raw_state", row.raw_state.clone())?;
            row_dict.set_item("normalized_state", row.normalized_state.clone())?;
            row_dict.set_item("order_quantity", row.order_quantity)?;
            row_dict.set_item("arriving_order", row.arriving_order)?;
            row_dict.set_item("inventory_before_demand", row.inventory_before_demand)?;
            row_dict.set_item("ending_inventory", row.ending_inventory)?;
            row_dict.set_item("period_cost", row.period_cost)?;
            row_dict.set_item("active_after_warmup", row.active_after_warmup)?;
            Ok(row_dict)
        })
        .collect::<PyResult<Vec<_>>>()?;
    dict.set_item("trace", rows)?;
    Ok(dict.into())
}
...
fn lost_sales_soft_tree_trace_from_demands(
    py: Python<'_>,
    flat_params: Vec<f32>,
    input_dim: usize,
    depth: usize,
    ...
    let warm_up_periods = warm_up_periods_for_horizon(demands.len(), warm_up_periods_ratio)?;
    let env_state = LostSalesState {
        current_inventory,
        lead_time_orders,
    };
    let (mean_cost, trace) =
        lost_sales_trace_from_demands_impl(&flat_params, &config, env_state, &demands)?;
    trace_payload_to_py(py, "soft_tree", mean_cost, warm_up_periods, &trace)
}
...
fn lost_sales_linear_trace_from_demands(
    py: Python<'_>,
    flat_params: Vec<f32>,
    input_dim: usize,
    output_dim: usize,
    ...
    let (mean_cost, trace) =
        lost_sales_linear_trace_from_demands_impl(&flat_params, &config, env_state, &demands)?;
    trace_payload_to_py(py, "linear", mean_cost, warm_up_periods, &trace)
}
...
fn lost_sales_nn_trace_from_demands(
    py: Python<'_>,
    flat_params: Vec<f32>,
    input_dim: usize,
    hidden_dims: Vec<usize>,
    ...
    let (mean_cost, trace) =
        lost_sales_neural_trace_from_demands_impl(&flat_params, &config, env_state, &demands)?;
    trace_payload_to_py(py, "nn", mean_cost, warm_up_periods, &trace)
}
...
pub fn register_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(
        lost_sales_soft_tree_trace_from_demands,
        m
    )?)?;
    ...
    m.add_function(wrap_pyfunction!(lost_sales_linear_trace_from_demands, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(lost_sales_nn_trace_from_demands, m)?)?;

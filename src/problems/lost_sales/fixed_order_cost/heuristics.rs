...
fn mean_after_warmup(epoch_costs: &[f64], warm_up_periods_ratio: f64) -> PyResult<f64> {
    ...
    let warm_up_periods = warm_up_periods_for_horizon(horizon, warm_up_periods_ratio)?;
...
pub struct FixedPolicyTraceRow {
    pub period: usize,
    pub demand: usize,
    pub pipeline_before_order: Vec<usize>,
    pub inventory_position_before_order: i64,
...
fn fixed_policy_order_quantity(
    policy_name: &str,
    params: &[usize],
    inventory_position: i64,
    max_order_size: usize,
...
pub fn fixed_policy_rollout_from_demands(
    ...
    for demand in demands.iter() {
        ...
        let order_quantity =
            fixed_policy_order_quantity(policy_name, params, inventory_position, max_order_size)?;
...
pub fn fixed_policy_trace_from_demands(
    policy_name: &str,
    params: &[usize],
    current_inventory: i64,
    lead_time_orders: &[usize],
    ...
    fixed_order_cost: f64,
    warm_up_periods_ratio: f64,
) -> PyResult<(f64, Vec<FixedPolicyTraceRow>)> {
    if lead_time_orders.is_empty() {
        return Err(PyValueError::new_err("lead_time_orders must be non-empty"));
    }
    ...
    for (period, demand) in demands.iter().enumerate() {
        let pipeline_before_order = lead_time_orders.clone();
        let inventory_position = inventory_position(current_inventory, &lead_time_orders);
        let order_quantity =
            fixed_policy_order_quantity(policy_name, params, inventory_position, max_order_size)?;
        ...
        lead_time_orders.push(order_quantity);
        current_inventory += arriving_order as i64;
        let inventory_before_demand = current_inventory;
        ...
        trace.push(FixedPolicyTraceRow {
            period,
            demand: *demand,

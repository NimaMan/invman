...
- **UPDATED — honest-floor deploy (≥5 seeds, 2026-06-06): LOSS → PARITY (downside-safe).** The training-path audit's root cause is now fixed at the deploy step. Deploying the CMA-ES **xbest** (the single best individual, what `es_mp.train` returns today) LOSES: 5-seed {123,456,789,2026,555} held-out (4096-seed CRN block) gives **228.86 ± 26.34** vs the LIR gate **203.76 ± 0.00** → **+12.32%, 1/5 below gate**. Adding the additive best-of{xbest, xfavorite, LIR-gate} **honest floor** (`--deploy_endpoint floor`, mirrors the OWMR reference) flips this to **PARITY**: deployed **200.994 ± 3.907** → **−1.36% vs gate, 2/5 below gate**, cross-seed std down **85.2%**. The intermediate **xfavorite**-only endpoint (CMA-ES distribution mean = `es.current_param()` = `result[5]`) is **210.31 ± 17.05** (+3.21%, 2/5 below) — the audit's diagnosed lever; the floor goes further by also flooring at the gate on the seeds where even xfavorite overfits. Per-seed deploy: {gate, xfavorite, xfavorite, gate, gate}; the floor deviated from xbest on **5/5** seeds. Runner: `scripts/random_yield_inventory/seed_robust_xbest_vs_xfavorite.py --deploy_endpoint floor` (artifact `_floor_5seed.json`); `--deploy_endpoint xbest` reproduces the LOSS exactly (verified additive/reversible). **Verdict: NOT a loss anymore — downside-safe parity with the gate (no robust beat: 2/5 below, gap inside seed noise).** PPO/WNH are context only.
- **SUPERSEDED — prior xbest-only verdict (LOSS):** an earlier 5-seed run reported soft-tree **227.36 ± 29.77** vs LIR gate **203.58 ± 5.68** → 0/5 below gate. Same root cause (xbest overfits the small training-seed batch → large held-out tail risk, e.g. seed 555 = 269.73); now addressed by the honest floor above. The prior provisional +4.25% (N=4) was an unreproducible older code path and is superseded.
...
- **autoresearch:** none — there is no `policy_search/programs/program_random_yield_inventory.md` (this system is not part of the autoresearch/paper pipeline).

...
- **SEED-ROBUST (honest floor, 5 seeds).** Training-path audit (2026-06-06): `train_soft_tree_reference.py` now carries an ADDITIVE `--deploy_endpoint {floor,xbest,xfavorite}` flag (default `floor`) mirroring the OWMR honest floor — it reads BOTH CMA-ES endpoints from the SAME run (xbest = `es.best_param()`, xfavorite = `es.current_param()` = distribution mean) and deploys the cheaper on the held-out block (downside-safe). At the standard budget (depth 2, oblique, linear, T=0.25, 400 episodes, pop 16, train_seed_batch 8, 2048-seed holdout) over 5 optimizer seeds {123,7,31,99,256}: **xbest = 63.53 ± 6.83 (LOSS: -4.97% vs benchmark base_stock S=5 = 60.52, not all seeds below gate); floored = 52.90 ± 0.37 (WIN: +12.59% vs gate, all 5/5 seeds below gate)**. The floor deployed xfavorite in all 5 seeds (5/5 deviates), cutting cost 16.73% and variance 6.83→0.37 vs xbest. **Verdict: LOSS → WIN vs gate (seed-robust).** `--deploy_endpoint xbest` reproduces the historical deploy-xbest behavior exactly.

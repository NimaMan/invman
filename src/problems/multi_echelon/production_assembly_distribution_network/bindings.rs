...
use crate::problems::multi_echelon::production_assembly_distribution_network::demand::{parse_demand_distribution_kind, DemandModel};
...
use crate::problems::multi_echelon::production_assembly_distribution_network::flownet::verification::verify_exact_reference_policy_performance;
use crate::problems::multi_echelon::production_assembly_distribution_network::heuristics::{
...
fn production_assembly_distribution_network_policy_rollout_from_paths(
    policy_name: &str,
    params: Vec<f64>,
    num_nodes: usize,
    source_nodes: Vec<bool>,
...
#[pyfunction]
fn production_assembly_distribution_network_pairwise_base_stock_requests(
    num_nodes: usize,
    source_nodes: Vec<bool>,
    node_modes: Vec<String>,
    external_supplier_lead_times: Vec<usize>,
...
#[pyfunction]
fn production_assembly_distribution_network_flownet_policy_verification_summary(py: Python<'_>) -> PyResult<PyObject> {
...
fn production_assembly_distribution_network_literature_benchmark_summary(
    py: Python<'_>,
...
pub fn register_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(production_assembly_distribution_network_soft_tree_rollout, m)?)?;
    m.add_function(wrap_pyfunction!(
        production_assembly_distribution_network_soft_tree_population_rollout,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        production_assembly_distribution_network_soft_tree_rollout_from_paths,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        production_assembly_distribution_network_policy_rollout_from_paths,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        production_assembly_distribution_network_pairwise_base_stock_requests,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        production_assembly_distribution_network_flownet_policy_verification_summary,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        production_assembly_distribution_network_literature_benchmark_summary,
        m
    )?)?;
    Ok(())
}

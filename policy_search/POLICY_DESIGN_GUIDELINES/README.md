...
- **Record results** in `outputs/autoresearch/<run_tag>/results.tsv` (one row per run:
  ...
  as a prior for the next run, and update `policy_search/README.md`. Do **not** re-litigate
  established priors.
...
8. **Write `program_<problem>.md` and the runner + TSV ledger**, mirroring an existing sibling
   (joint_replenishment is the closest template), and register it in `policy_search/README.md`.
...
- **Autoresearch + report (step 8-9).** No
  `policy_search/programs/nonstationary_lot_sizing/README.md` exists yet
  (the documented gap vs OWMR/JR/VMI). Build a lightweight one mirroring
  `policy_search/programs/joint_replenishment/README.md`: ledger the head sweep (direct-order vs

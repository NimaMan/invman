...
4. Lookups:
   ...
   - `render_all_cards(out_dir)` -> writes one card per problem to
     `<out_dir>/<problem>/README.md` plus a `README.md` index, returns the dict
     of written paths.
...
docs/benchmarks/VERIFICATION_LEDGER/README.md + FUNDAMENTAL_QUESTIONS/README.md; the ledger wins)
----------------------------------------------------------
...
def _read_tier(entry: dict[str, Any]) -> str:
    ...
    reconciled against docs/benchmarks/VERIFICATION_LEDGER/README.md +
    FUNDAMENTAL_QUESTIONS/README.md. It is NOT derived from the prose status string
...
def render_all_cards(out_dir: str | Path) -> dict[str, Path]:
    """Write one card per problem to `<out_dir>/<problem>/README.md` + an index.
    ...
    cards = all_cards()
    for card in cards:
        card_dir = out_path / card.problem
...
def _render_index(cards: list[ProblemCard]) -> str:
    ...
    for card in cards:
        lines.append(
            ...
            f"| `{card.verification.tier}` | [{card.problem}](./{card.problem}/) |"
        )
    lines.append("")

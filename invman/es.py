...
class PEPG:
    ...
    def __init__(
        ...
        # option to use greedy es_algos method to select next mu, rather than using drift param
        self.elite_ratio = elite_ratio
        self.elite_popsize = int(self.popsize * self.elite_ratio)
...
class XNES:
    ...
    def __init__(
        ...
        # xnes parameters
        self.eta_mu = eta_mu
        self.use_adasam = True
        self.dim = num_params
        self.sigma = np.abs(np.linalg.det(self.A)) ** (1.0 / self.dim)
        self.B = self.A / self.sigma
        self.eta_sigma = (
            3 * (3 + np.log(self.dim)) * (1.0 / (5 * self.dim * np.sqrt(self.dim)))
    ...
    def ask(self):
        ...
        self.solutions = self.mu.reshape(1, self.num_params) + self.sigma * np.dot(self.epsilon, self.B.T)
    ...
    def tell(self, reward_table_result):
        ...
        if self.use_adaptive_sampling and self.sigma_old is not None:  # sigma_old must be available
            self.eta_sigma = self.adaptive_sampling(self.eta_sigma, self.mu, self.sigma, self.B, self.sigma_old, self.solutions)
        delta_delta = np.dot(normalized_reward, self.epsilon)
        delta_M = np.dot(self.epsilon.T, self.epsilon*normalized_reward.reshape(self.popsize, 1)) - np.sum(normalized_reward)*np.eye(self.dim)
        delta_sigma = np.trace(delta_M) / self.dim
        delta_B = delta_M - delta_sigma * np.eye(self.dim)

        # update
        self.mu += self.eta_mu * self.sigma * np.dot(self.B, delta_delta)
        self.sigma *= np.exp(0.5 * self.eta_sigma * delta_sigma)
        self.B = np.dot(self.B, sp.linalg.expm(0.5 * self.eta_B * delta_B))

        # logging
        self.history['fitness'].append(self.best_reward)
        self.history['sigma'].append(self.sigma)
        self.history['eta_sigma'].append(self.eta_sigma)
    ...
    def adaptive_sampling(self, eta_sigma, mu, sigma, B, sigma_old, z_try):
        """ Adaptation sampling """
        ...
        p0 = sp.stats.multivariate_normal.logpdf(z_try, mean=mu, cov=cov)
        p1 = sp.stats.multivariate_normal.logpdf(z_try, mean=mu, cov=cov_)
        w = np.exp(p1 - p0)
        ...
        cum = sp.stats.norm.cdf(u_, loc=u_mu, scale=u_sigma)

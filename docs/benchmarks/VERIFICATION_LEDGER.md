...
| faithful_unverified (only repo-native self-consistency re-ran; or env faithful but no published cost) | 6 | ameliorating_inventory; joint_pricing_inventory; procurement_removal_inventory; random_yield_inventory; spare_parts_inventory/trainable-env; vendor_managed_inventory |
| snapshot_only_not_rerun (asserted literals, NOT executed — DEBT) | 4 items | divergent A3C relative rows; gbk set2/set3 (+223%); van_oers 2024 Table 1 (padn/spare-parts adjacent); dual_sourcing l_r=3,4 (prior-date validated, not re-run this audit) |
...
|---|---|---|---|---|
| **ameliorating_inventory** | Perfect-info LP bounds: spirits_0001 1991.934, port_wine 2444.801, spirits_1002 1663.889 | LP bound **NOT re-run** (no Python binding; needs cargo). In-crate test re-solves LP within 1e-3 (read, not executed). Learned env path re-ran (spirits_0001 smoke 77.96 vs gate 20.07). | faithful_unverified (bound not executed; env path executed) | learned: `autoresearch_ameliorating_inventory_average_profit.py --instance spirits_0001 --budget smoke`; bound: `cargo test ...verification` |
...
| **multi_echelon / assembly** | Verified-by-equivalence only (Rosling reduction to serial); literature_verified=false on every instance; **could not re-run via bindings** (no assembly env / reduction binding) — manual remap gave 26.55 vs 22.759. | n/a this audit (Rust-only path). |
...
| D2 | **gbk set2 / set3 (+223%)** carried as table-only published rows | multi_echelon/general_backorder_fixed_cost references.rs | The order-per-edge / restricted-transition spec exists only in the gated CEJOR full text; +223% means NOT reproduced. | Recover the transition spec or mark explicitly NOT-reproduced; do not present alongside the verified set1/KT rows without a flag. |
...
Also note (not snapshot, but missing re-run): **ameliorating_inventory perfect-info LP bound** has no Python binding and was not executed this audit (the in-crate re-solve test is genuine but Rust-only). Add `ameliorating_inventory_perfect_information_upper_bound` so the bound is re-runnable in <2 min — this would upgrade ameliorating from faithful_unverified to verified_rerun.

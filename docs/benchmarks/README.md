...
| **multi_echelon / assembly** | 1 (3 sub) | ✓ | ✓ Rosling reduction → serial solver | faithful_unverified — structurally anchored by Rosling reduction; all instances `literature_verified=false`; independent Python verifier passes but there is no learned-policy binding | — (no dedicated section) | [card](../../src/problems/multi_echelon/assembly/BENCHMARK.md) |
...
Most "beats heuristic/gate" headlines outside the newly re-run families are still a **single optimizer seed or best-of-N** and are labeled NOT-yet-seed-robust in their cards:

...
def main() -> int:
    ...
    instances = [
        (name, rust.lost_sales_reference_costs(name))
        for name in rust.lost_sales_reference_instance_names()
        if name != "vanilla_l4_p4_poisson5"
    ]
    # cheapest first: ascending lead time, then name
    instances.sort(key=lambda ni: (int(ni[1]["lead_time"]), ni[0]))

    for name, inst in instances:
        params = {
            "demand_dist_name": inst["demand_kind"],
            "demand_rate": inst["demand_rate"],
            "demand_lambda_low": inst.get("demand_lambda_low", 0.0),
            "demand_lambda_high": inst.get("demand_lambda_high", 0.0),
            "demand_p00": inst.get("demand_p00", 0.0),
            "demand_p11": inst.get("demand_p11", 0.0),
            "lead_time": inst["lead_time"],
            "holding_cost": inst["holding_cost"],
            "shortage_cost": inst["shortage_cost"],
        }

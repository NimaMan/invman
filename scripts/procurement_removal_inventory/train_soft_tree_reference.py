...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a Rust-backed soft-tree policy on the procurement_removal_inventory primary reference instance."
    )
...
def _training_namespace(parsed, reference):
    run_tag = (
        f"procurement_removal_{reference['name']}_d{parsed.depth}_{parsed.split_type}_{parsed.leaf_type}"
        f"_s{parsed.seed}_b{parsed.train_seed_batch}"
    )
    output_root = PACKAGE_ROOT / "outputs" / "procurement_removal_inventory" / run_tag
...
def _get_population_fitness(model, reference):
    def inner(_model, args, model_params_batch, seeds):
        ...
        for seed_offset in range(int(getattr(args, "train_seed_batch", 1))):
            batch_costs.append(
                invman_rust.procurement_removal_inventory_soft_tree_population_rollout(
...
def _comparison_table(reference: dict, soft_tree_eval: dict) -> list[dict]:
    interval = evaluate_heuristic_policy(reference, "interval_stock", range(100000, 100000 + int(soft_tree_eval["num_samples"])))
    buffer = evaluate_heuristic_policy(
        reference,
        "returnability_buffer_interval_stock",
        range(100000, 100000 + int(soft_tree_eval["num_samples"])),
    )
    rows = [
        {
            "policy": "interval_stock",
            "params": interval_stock_params(reference),
            "mean_cost": float(interval["mean_discounted_cost"]),
            "note": "benchmark heuristic",
        },
        {
            "policy": "returnability_buffer_interval_stock",
            "params": returnability_buffer_params(reference),
            "mean_cost": float(buffer["mean_discounted_cost"]),
            "note": "benchmark heuristic",
        },
        {
            "policy": "soft_tree",

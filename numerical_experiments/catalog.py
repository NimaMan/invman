...
EXPERIMENT_SUITES: tuple[ExperimentSuite, ...] = (
    ...
    ExperimentSuite(
        ...
        script_args=(
            ...
            "0.75",
            "--description",
            "numerical_experiments_gijs_setting2",
        ),
        notes=(
            "Runs the paper-faithful gijs_2022 setting via the CMA-ES autoresearch runner (default gijsbrechts2022_setting2; pass --reference gijsbrechts2022_setting1 for setting 1).",
            "Compares the learned soft-tree to the in-env best constant base-stock; the relative improvement is the comparator to the published A3C savings (8.95% / 12.09%). The absolute Van Roy costs reproduce on the separate van_roy1997_case_study* instances.",
        ),
    ),
)
